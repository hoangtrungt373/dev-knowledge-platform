 ...
 * Keycloak UUID.
 *
 * <p>{@code ProjectApi}/{@code TaskApi} take {@code @CurrentUserId String ownerUuid} directly (not
 * {@code @AuthenticationPrincipal CustomOAuth2User}). {@link CurrentUserResolver} is now a shared
 * {@code infra} class (see its own Javadoc) rather than a local copy — this module's own copy used
 * to exist under a module-specific method name ({@code resolveOwnerUuid}) before that
 * consolidation; the shared class exposes {@code resolveUserUuid} instead, called below and
 * assigned to this module's own {@code ownerUuid} vocabulary at the call site.
 *
