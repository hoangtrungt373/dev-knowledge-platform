...
(update/avatar). Package root: `com.ttg.devknowledgeplatform.identity.*`. Extracted from `gateway`
(named `api` at the time) as one of several parallel workstreams peeling REST controllers/DTOs/
mappers out of that module into feature modules — see root `CLAUDE.md`'s Module Structure and
`docs/CHANGELOG.md` for the "why now."
...
- `api/` (+ `api/impl/`) — `OAuth2Api`/`OAuth2Controller` (moved here in full: oauth2Authorization,
  ...
  **split-off** `UserApi`/`UserController` containing only `updateProfile`/`uploadAvatar` — the pure
  ...
  `getPublicProfile` and `search` moved to `social-service`'s own `UserApi`/`UserController` instead
  (same `/api/v1/users` mapping, different class) because they need `FriendService`/`FriendMapper`/
  `RelationshipStatus` for relationship enrichment — `social-service` reaches into this module's
  `UserService`/`UserMapper` for the base lookup rather than the other way around, since this module
  must never depend on `social-service` (this module stays a pure leaf: `common`+`infra` only, no
...
- `dto/auth/` — `LoginRequest`, `LoginResponse`, `RegisterResponse`, `ExchangeStateRequest`,
  ...
  behavior, same shape as when they lived in `gateway`.
...
- `security/service/UserService` (+ `impl/`) — registration (OAuth2 + local), password hashing,
  ...
  DTOs, so `social-service`'s `UserController` (the `getPublicProfile`/`search` half) can keep
  calling it directly across the module boundary. `CustomOAuth2UserService`/`CustomOidcUserService` — find-or-create on OAuth2/OIDC
...
- **`CacheNames`/`CacheTtlProperties` live in `infra`, not here or in `gateway`** — they're shared by
  this module's `StateTokenServiceImpl` and `gateway`'s `RedisCacheConfig`, two modules that can't
  depend on each other, so they moved to `infra` (the "utility needed by two modules" case its own
