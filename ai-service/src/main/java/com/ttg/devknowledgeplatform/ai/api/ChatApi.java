 ...
 * live here. The implementation class ({@link com.ttg.devknowledgeplatform.ai.api.impl.ChatController}) contains only delegation
...
public interface ChatApi {
     ...
     * @param userUuid authenticated user's Keycloak subject id, injected from the JWT
     ...
     */
    @GetMapping("/sessions")
    ResponseEntity<List<ChatSessionSummaryDto>> listSessions(@CurrentUserId String userUuid);

    /**
     * Returns the full message history for a session.
     *
     * @param id     session primary key
     * @param userUuid used to verify ownership — returns {@code 404} if the session belongs to another user
