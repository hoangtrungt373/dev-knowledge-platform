...
import { EmbeddingIndexItem, EmbeddingContentType } from '../../types/admin.types';
...
export default function EmbeddingsPage(): JSX.Element {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const result = await adminApi.listEmbeddings({
        page,
        size: rowsPerPage,
        q: q || undefined,
  ...
  const handleConfirm = async () => {
    ...
    try {
      switch (pendingAction.kind) {
        case 'reindex':
          await adminApi.reindexContent(pendingAction.contentItemId!, showError);
          showSuccess('Re-indexing complete.');
          break;
        case 'deleteIndex':
          await adminApi.deleteContentIndex(pendingAction.contentItemId!, showError);
          showSuccess('Embedding index deleted.');
          break;
        case 'indexAll':
          await adminApi.indexAll(showError);
          showSuccess('Bulk indexing started.');
          break;
        case 'refreshCorpus':
          await adminApi.refreshCorpus(showError);
