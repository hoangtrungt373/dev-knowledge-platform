...
export default function QuestionAnswerFormPage(): JSX.Element {
  ...
  useEffect(() => {
    Promise.all([
      contentApi.getCategoryTree(showError),
      contentApi.listTags({ size: 1000, sortBy: 'name', sortDir: 'asc' }, showError),
    ]).then(([nodes, tagsPage]) => {
      setTreeNodes(nodes);
    ...
    if (isEdit && id) {
      contentApi.getQuestionAnswer(Number(id), showError)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && id) {
        await contentApi.updateQuestionAnswer(Number(id), common as UpdateQuestionAnswerPayload, showError);
        showSuccess('Question updated');
      } else {
        await contentApi.createQuestionAnswer(common as CreateQuestionAnswerPayload, showError);
