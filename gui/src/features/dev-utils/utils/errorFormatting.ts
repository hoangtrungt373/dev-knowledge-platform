 ...
 * `JSON.parse` produces an even more precise, human-readable syntax error (native V8 message, e.g.
 * "Expected ',' or '}' after property value in JSON at position 81 (line 1 column 82)") than
 * Jackson's own phrasing ever could — reused verbatim instead of asking the backend at all, since
 * it's strictly better and doesn't need a round trip; (2) `simplifyBackendMessage` stays as a
 * defensive fallback for `yaml-to-json`/`html-beautify` (no client-side parser available for
 * either) — normally a pure passthrough of the now-already-clean backend message, but still
 * tolerant of the old noisy shape too, in case this ever regresses or a genuinely different
 * technical-error message (network failure, 5xx) reaches it instead.
 */
