...
import LoremIpsumPanel from '../components/LoremIpsumPanel';
        ...
        <Box component="main" ref={mainColumnRef} sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Box ref={toolPanelRef}>
            {/* Some operations render their own bespoke layout instead of the shared
                ...
                Output, QR Code Reader/Generator's own text-field-plus-image-upload Input/
                rendered-QR-or-decoded-text Output, and Lorem Ipsum Generator's own
                paragraph-count-slider Input (no free text at all) all don't fit that component's
                plain Input/Output editor pair at all. HTML Preview and Markdown Preview are two
                ...
                nine custom-layout components today; extend this the same way (one more `else
            ...
            {activeOperation.key === 'hash-generator' ? (
              ...
              <QrCodePanel
                ...
                availableHeight={panelHeight}
              />
            ) : activeOperation.key === 'lorem-ipsum' ? (
              <LoremIpsumPanel
                key={activeOperation.key}
                input={input}
                onInputChange={setInput}
