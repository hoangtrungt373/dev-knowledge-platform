 ...
 * refunding it is Epic 4's job, same deferral {@code ConfirmedOrderStatusHandler#cancel} already
 * documents); one that resolves through {@link #failPayment} needed the exact same {@code release}
    ...
    public void confirmPayment(Order order) {
        ...
        if (Boolean.TRUE.equals(order.getCancelRequested())) {
            ...
            OrderStatusTransitions.transitionTo(order, OrderStatus.CANCELLED,
                    "Payment succeeded while a cancellation was pending — refund handling is "
                            + "deferred to Epic 4's payment-gateway integration");
        } else {
