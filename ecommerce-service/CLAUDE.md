...
Only **Epic 1 (Catalog & Search)** has code so far. It now has a complete admin CRUD slice for
`ProductCategory`/`Product`, the outbox relay, and a public browse/search endpoint — see below.
The plan is to keep rounding out Epic 1, then extract `ecommerce-service` into its own standalone
Spring Boot application (own DB, own JWT validation, `gateway`-proxied) as a dedicated
microservices-study exercise — see the `project-ecommerce-service-module` memory for the full
sequencing. Nothing built so far should assume it will always run inside the monolith.
...
- `ProductSearchView` — the CQRS read model for browse/search/filter; **written only by the
  ...
  built now). Its `SEARCH_VECTOR` (`tsvector`) column is DB-generated
...
(per this repo's convention, lives under `gateway`'s changelog tree, not a folder of its own).
...
- `api/` (+ `api/impl/`) — `ProductCategoryApi`/`Controller` (`/api/v1/admin/product-categories`:
  ...
  `PATCH .../deactivate`/getById/paginated-list). Both admin-gated automatically via `gateway`'s
  existing `SecurityConfig` URL-pattern rule for `/api/v1/admin/**` — no method-level
  `@PreAuthorize` needed, same as `content-service`'s `CategoryApi`.
...
- `outbox/OutboxRelay` — the `@Scheduled` poller (`app.ecommerce.outbox.relay.poll-interval`,
  ...
  configurable `@Scheduled` intervals). No `@EnableScheduling` here — it's already active
...
The public browse/search endpoint (US-1.1, US-1.3, US-1.4):
...
- `repository/ProductSearchViewRepository.search` — one native query (`nativeQuery = true`)
  ...
  check (`MAX_PRICE >= :minPrice AND MIN_PRICE <= :maxPrice`), not exact match. The `SELECT` list
  is spelled out explicitly, never `SELECT *` — `SEARCH_VECTOR` is a DB-only generated column
  with no Java field, and an unmapped column returned by `SELECT *` would break the entity-result
  mapping.
- `service/ProductSearchService`(`Impl`) — thin: resolves the trigram threshold constant, blanks
  out an empty `q`, and calls the repository with an **unsorted** `PageRequest` — the native
  query already bakes in its own `ORDER BY`, so a `Sort` here would make Spring Data append a
  second, conflicting one.
- `mapper/ProductSearchViewMapper`, `dto/ProductSearchResponse`,
  `api/ProductSearchApi`+`Controller` at `/api/v1/public/products` — under `gateway`'s existing
  `/api/v1/public/**` permit-all rule, no auth required, matching the public nature of browsing.

**Not built yet** (do not assume these exist): variant/image add-remove-reorder endpoints
(`ProductService.update` is basic-fields-only for now), `ProductCategory` delete, a public
product-detail endpoint (US-1.2 — straightforward to add later by exposing `ProductService.getById`
publicly, just not done yet), full attribute-value (size/color) filtering on the search endpoint
beyond what `ProductSearchView.availableAttributes` already carries for display, and everything
for Epics 2–5. Check `docs/CHANGELOG.md`'s `[Unreleased]` entry and this file's own freshness
before assuming more exists than what's listed above. **None of this has been compiled/verified
yet** — treat it as written-but-unverified until it's actually been built and run.

## Rules specific to this module

- **Depends only on `common` + `infra` today.** Epic 5 (Reviews & Recommendations) will add a new
  one-directional dependency on `ai-service` (for embedding generation, owning its own
  `ProductEmbedding` table with an FK to this module's own `Product`) — see
  `docs/user-stories/05-reviews-recommendations.md` for the reasoning (keeps `ai-service`'s own
  dependency count at one, still just `content-service`). Do not add that dependency before Epic 5
  actually needs it.
- **Table names must not collide with any existing table in the shared `product` schema.**
  `ProductCategory`/`PRODUCT_CATEGORY` exists specifically to avoid colliding with
  `content-service`'s `Category`/`CATEGORY` — check `docs/PROJECT_STRUCTURE.md` for the full table
...
- **Every `OutboxEventHandler` with a non-trivial payload also declares its own nested
  ...
  exactly one place.
- Liquibase migrations for this module's tables live under `gateway`'s changelog tree, same as
  every other feature module — see root `CLAUDE.md`'s Database Conventions. Don't create a
  changelog folder here.
