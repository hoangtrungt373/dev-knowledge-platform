 ...
 * format, PHP→JSON, JSON→PHP, JWT Debugger, URL Parser. Shared across every one of those because
 * they genuinely share this shape (raw text in, a minify flag, transformed text out); an
 * operation whose input/output doesn't fit this shape (e.g. JSON→YAML/JSON→CSV/String Case
 * Converter, none of which has a minify concept at all — see {@link TextRequest}'s own Javadoc —
 * or a future Unix Time
